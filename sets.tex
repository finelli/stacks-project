...
a complete formula $\phi_\textup{scheme}(x)$ with one free variable $x$ in set theory
...
$\phi_\textup{scheme}(X)$ expresses the property ``$X$ is a scheme'', then
what does the formula $\phi_\textup{scheme}^{V_\alpha}(X)$ mean?
It is true that the reflection principle says we can find $\alpha$ such that
for all $X \in V_\alpha$ we have
$\phi_\textup{scheme}(X) \leftrightarrow \phi_\textup{scheme}^{V_\alpha}(X)$
